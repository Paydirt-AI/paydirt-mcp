...
5. Give the returned `authorization_url` to the user. This browser flow handles Google sign-in, app and form creation, and the required delivery choice: Slack and coding agents (recommended), Slack only, or coding agents only. When Slack is selected, require Slack OAuth, provision `#paydirt-suggest-a-feature` and `#paydirt-cancellations`, route feature forms to the first channel and trial/subscription cancellation forms to the second, and verify every assignment before completing setup. Do not poll, sleep, or claim setup is finished while authorization is pending.
...
8. Use the returned `ios.install_verification` contract to add a DEBUG-only one-time app-ready trigger, launch the built app on an available simulator or connected development device, and leave the Paydirt setup check visibly open. The developer personally submits Suggest a Feature, Trial Cancellation, and Subscription Cancellation. For Slack delivery, completion requires 3/3 verified deliveries: the feature test in `#paydirt-suggest-a-feature` and both distinctly labeled cancellation tests in `#paydirt-cancellations`. Never submit a test for them and never include the automatic trigger in release builds.
9. Respect the delivery preference returned by setup; do not ask again unless the developer requests a change. When Slack was selected, verify the two typed channels and every form assignment. Agent access is read-only: do not inject every response into every session or take automatic action.
